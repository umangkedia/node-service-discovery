            ...
            setInterval(function () {
                self.getServicesDataFromZK(false, function (error) {
                    if (error)
                        ...
                        logger.debug("refreshed services data from zookeeper for:", self.serviceName);
                })
 ...
 * Also, checks the lastUpdatedTimeStamp of node and removes if diff is more than specified value
 *
 * @public
 * @method saveNodeData
 *
...
InstanceProvider.prototype.saveNodeData = function (nodeData, absPath, self, callback) {
    ...
    nodeData.forEach(function (data) {
        if (data.healthcheckStatus === "healthy"
            && (currentTimestamp - data.lastUpdatedTimeStamp <= self.serviceDiscovery.staleNodeTimeDiff)) {
            updateNodeData.push(data);
        }
        else
            logger.info("node removed due to health check/stale value:", JSON.stringify(data), self.serviceName, currentTimestamp);
    });
