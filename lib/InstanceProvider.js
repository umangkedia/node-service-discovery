 ...
 *
 * @param watcher           {Boolean}    Variable to determine if watcher needs to be set or otherwise
 * @param callback          {Function}   Optional callback function
 *
 * @public
...
InstanceProvider.prototype.getServicesDataFromZK = function (watcher, callback) {
    ...
    async.waterfall([
        function (callback) {
            ...
            callback(null, absPath, watcher, self);
        },
 ...
 * @param that          {Object}    The parent reference from where ever this method is called via async
 */
InstanceProvider.prototype.watcher = function (event, that) {
    var self = that;
 ...
 * @param watcher       {Boolean}   Variable to determine if the watcher needs to be set or otherwise
 * @param that          {Object}    The parent reference from where ever this method is called via async
 * @param callback      {Function}  Optional callback function
 *
 */
InstanceProvider.prototype.getNodeList = function (absPath, watcher, that, callback) {
    var self = that;
    var watcherCallback = !watcher ? null : function (event) { self.watcher(event, self) };
    ...
    self.serviceDiscovery.client.getZkClient().getChildren(
        ...
        function (error, serviceList, stat) {
            if (!error && serviceList.length > 0) {
                ...
                return callback(null, absPath, serviceList, that);
            }
 ...
 *
 */
InstanceProvider.prototype.getNodeData = function (absPath, serviceList, that, callback) {
    var self = that;
    var nodeData = [];
    ...
    async.each(serviceList, function (service, intCallback) {
        ...
        callback(null, nodeData, absPath, that);
    });
};
 ...
 * @returns {*}
 */
InstanceProvider.prototype.saveNodeData = function (nodeData, absPath, that, callback) {
    var self = that;
