 ...
 */
function ServiceDiscovery(client, basePath, serviceInstance, refreshInterval) {
    this.client = client;
...
ServiceDiscovery.prototype.registerService = function (callback) {
    ...
    //3. If a callback is bound, relay the error/response via the callback
 ...
 *
 * @public
 * @method unRegisterSerice
 *
 * @param serviceName       {String}    The service that has to be unregistered
 * @param callback          {Function}  An optional callback to be called upon un-register
 */
ServiceDiscovery.prototype.unRegisterService = function (serviceName, callback) {
    var servicePath = [self.basePath, self.serviceInstance.name, serviceName].join('/');
